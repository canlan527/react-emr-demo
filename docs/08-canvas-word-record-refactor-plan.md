...
- 平时仍保持 `textarea` 不可见，避免和 Canvas 正文形成双份文字。
- 在 composition 期间临时显示输入代理，让用户看到拼音预编辑串。
- 输入代理的 `font-family`、`font-size`、`line-height` 应尽量和当前 Canvas 行一致。
- 后续进入富文本后，IME 输入代理需要能根据当前 run 的字号动态更新样式。
